...
import time
from Modules.MLP_add_interaction_self_different_channel_with_first_sent import ourModel, train_epoch, valid_epoch, test_process
from utils.dataset_first_sent import DoctorRecDataset
...
def main():
    ...
    first_sent = load_pickle(f'{args.cleaned_path}/first_sent_embeds.pkl')
    print("Loaded first_sents embeddings")
    ...
    print("The length of dialogues: ", len(first_sent))
    ...
    train_dataset = DoctorRecDataset(
        'train', train_set, profile, query, dialogue, first_sent,
    ...
    val_dataset = DoctorRecDataset(
        'valid', valid_set, profile, query, dialogue, first_sent,
        dr_dialog_sample=args.dr_dialog_sample, neg_sample=args.neg_sample
    )
