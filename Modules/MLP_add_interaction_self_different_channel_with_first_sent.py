...
class cnnBlock(nn.Module):
    def __init__(self):
        ...
        self.cnn_2d_query_profile_1 = nn.Conv2d(in_channels=3, out_channels=8, kernel_size=(3,3))
        ...
        self.affine_query_profile = nn.Linear(in_features=6272, out_features=1000)
        self.relu = nn.ReLU()
        self.leaky_relu = nn.LeakyReLU()
        self.tanh = nn.Tanh()

        self.cnn_2d_query_dialogs_1 = nn.Conv2d(in_channels=3, out_channels=8, kernel_size=(3,3))
        ...
        self.affine_query_dialogs = nn.Linear(in_features=4480, out_features=1000)

        self.cnn_2d_query_first_sent_1 = nn.Conv2d(in_channels=3, out_channels=8, kernel_size=(3,3))
        self.cnn_2d_query_first_sent_2 = nn.Conv2d(in_channels=8, out_channels=16, kernel_size=(3,3))
        self.cnn_2d_query_first_sent_3 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=(3,3))
        self.maxpooling_query_first_sent_1 = nn.MaxPool2d(kernel_size=(2, 2), stride=(2, 2))
        self.affine_query_first_sent = nn.Linear(in_features=4480, out_features=1000)
        
        self.bn1 = nn.BatchNorm1d(num_features=768)
        self.fc1 = nn.Linear(in_features =1000 * 3, out_features = 768)
    ...
    def cnn_query_first_sent(self, matrix):
        Z = self.leaky_relu(self.cnn_2d_query_first_sent_1(matrix))
        Z = self.maxpooling_query_first_sent_1(Z)
        # print("cnn_query_first_sent Z1: ", Z)
        
        Z = self.leaky_relu(self.cnn_2d_query_first_sent_2(Z))
        Z = self.maxpooling_query_first_sent_1(Z)
        # print("cnn_query_first_sent Z2: ", Z)

        Z = self.relu(self.cnn_2d_query_first_sent_3(Z))
        Z = self.maxpooling_query_first_sent_1(Z)
        # print("cnn_query_first_sent Z3: ", Z)

        Z = Z.view(Z.size(0), -1)

        output_V = self.affine_query_first_sent(Z)
        return output_V

    def forward(self, query_profile_similarity_matrix, query_dialogs_similarity_matrix, query_first_sent_similarity_matrix, \
                query_profile_self_attn_similarity_matrix, query_dialogs_self_attn_similarity_matrix,query_first_sent_self_attn_similarity_matrix, \
                query_profile_interaction_simialrity_matrix, query_dialogs_interaction_simialrity_matrix, query_first_sent_interaction_simialrity_matrix):
        ...
        query_first_sent_M = torch.stack([query_first_sent_similarity_matrix, query_first_sent_self_attn_similarity_matrix, query_first_sent_interaction_simialrity_matrix], dim=1)
        ...
        query_first_sent_V = self.cnn_query_first_sent(query_first_sent_M)
        ...
        features = torch.cat([query_profile_V, query_dialogs_V, query_first_sent_V], dim=-1)
...
class ourModel (nn.Module):
    ...
    def forward(self, batch_query_emb, batch_profile_emb, batch_dialogs_emb, batch_first_sent_emb, \
                batch_query_mask, batch_profile_mask, batch_dialogs_mask, batch_first_sent_mask):
        ...
        batch_first_sent_emb = batch_first_sent_emb.masked_fill(batch_first_sent_mask.unsqueeze(-1)==0, 0)
        ...
        batch_first_sent_emb_norm = torch.nn.functional.normalize(batch_first_sent_emb, dim=-1) 
        ...
        query_first_sent_similarity_matrix = torch.bmm(batch_query_emb_norm, batch_first_sent_emb_norm.transpose(1,2))
        ...
        first_sent_self_attn_mask_ = torch.bmm(batch_first_sent_mask.unsqueeze(-1), batch_first_sent_mask.unsqueeze(1)).bool()
        ...
        batch_first_sent_self_attn = self.dialogs_transformer(batch_first_sent_emb, batch_first_sent_emb, batch_first_sent_emb, mask=first_sent_self_attn_mask_)
        batch_first_sent_self_attn_norm = torch.nn.functional.normalize(batch_first_sent_self_attn, dim=-1) 
        ...
        query_first_sent_self_attn_similarity_matrix = torch.bmm(batch_query_self_attn_norm, batch_first_sent_self_attn_norm.transpose(1,2))
        ...
        query_first_sent_attn_mask_ = torch.bmm(batch_query_mask.unsqueeze(-1), batch_first_sent_mask.unsqueeze(1)).bool()
        ...
        query_add_first_sent_attn = self.query_transformer(batch_query_emb, batch_first_sent_emb, batch_first_sent_emb, mask=query_first_sent_attn_mask_)
        query_add_first_sent_attn_norm = torch.nn.functional.normalize(query_add_first_sent_attn, dim=-1) 

        first_sent_add_query_attn = self.dialogs_transformer(batch_first_sent_emb, batch_query_emb, batch_query_emb, mask=query_first_sent_attn_mask_.transpose(1,2))
        first_sent_add_query_attn_norm = torch.nn.functional.normalize(first_sent_add_query_attn, dim=-1)
        ...
        query_first_sent_interaction_simialrity_matrix = torch.bmm(query_add_first_sent_attn_norm, first_sent_add_query_attn_norm.transpose(1,2))
        
        output = self.cnn_block(query_profile_similarity_matrix, query_dialogs_similarity_matrix, query_first_sent_similarity_matrix, \
                                query_profile_self_attn_similarity_matrix, query_dialogs_self_attn_similarity_matrix,query_first_sent_self_attn_similarity_matrix, \
                                query_profile_interaction_simialrity_matrix, query_dialogs_interaction_simialrity_matrix, query_first_sent_interaction_simialrity_matrix)
...
def train_epoch(train_dataloader, optimizer, model, tag):
    ...
    for batch, labels in tqdm(train_dataloader):
        ...
        batch_first_sent_embed, batch_first_sent_attention_mask = batch[6], batch[7].float()
        ...
        batch_first_sent_mask = model.get_dialog_sent_masks(batch_first_sent_attention_mask).float()

        if tag == "train":
            optimizer.zero_grad()
        logits = model(batch_query_embed, batch_profile_embed, batch_dialogs_embed, batch_first_sent_embed, \
                        batch_query_attention_mask, batch_profile_attention_mask, batch_dialogs_mask, batch_first_sent_mask)
...
def test_process(test_dataloader, model):
    ...
    for batch, labels in tqdm(test_dataloader):
        ...
        batch_first_sent_emb, batch_first_sent_attention_mask = batch[6], batch[7].float()
        # print("batch_dialogs_attnetion_mask", batch_dialogs_attention_mask.shape)
        ...
        logits = model(batch_query_emb, batch_profile_emb, batch_dialogs_emb, batch_first_sent_emb, \
                        batch_query_attention_mask, batch_profile_attention_mask, batch_dialogs_mask, batch_first_sent_mask)
