...
class ourModel (nn.Module):
    ...
    def forward(self, batch_query_emb, batch_profile_emb, batch_dialogs_emb, \
        ...
        batch_query_self_attn = self.query_self_transformer(batch_query_emb_norm, batch_query_emb_norm, batch_query_emb_norm, query_self_attn_mask_)
        ...
        # 2.1 Calcuate attn
        query_add_profile_attn = self.query_profile_transformer(batch_query_self_attn, batch_profile_self_attn, batch_profile_self_attn, mask=~query_profile_attn_mask_)
        profile_add_query_attn = self.profile_transformer(batch_profile_self_attn, batch_query_self_attn, batch_query_self_attn, mask=~query_profile_attn_mask_.transpose(1,2))

        query_add_dialogs_attn = self.query_dialogs_transformer(batch_query_self_attn, batch_dialogs_self_attn, batch_dialogs_self_attn, mask=~query_dialogs_attn_mask_)
        dialogs_add_query_attn = self.dialogs_transformer(batch_dialogs_self_attn, batch_query_self_attn, batch_query_self_attn, mask=~query_dialogs_attn_mask_.transpose(1,2))
