...
class cnnBlock(nn.Module):
    def __init__(self):
        ...
        self.affine_query_profile_interaction = nn.Linear(in_features=15625, out_features=2000)
        ...
        self.affine_query_dialogs_interaction = nn.Linear(in_features=12125, out_features=2000)
    ...
    def init_weights(self):
        ...
        init.xavier_normal_(self.affine_query_profile_interaction.weight)
        ...
        init.xavier_normal_(self.affine_query_dialogs_interaction.weight)
    ...
    def forward(self, query_profile_similarity_matrix, query_dialogs_similarity_matrix, \
        ...
        query_profile_Z = self.cnn_query_profile(query_profile_similarity_matrix)
        query_profile_V = self.affine_query_profile(query_profile_Z)

        query_profile_interaction_Z = self.cnn_query_profile(query_profile_interaction_simialrity_matrix)
        query_profile_interaction_V = self.affine_query_profile_interaction(query_profile_interaction_Z)

        query_dialogs_Z = self.cnn_query_dialogs(query_dialogs_similarity_matrix)
        query_dialogs_V = self.affine_query_dialogs(query_dialogs_Z)

        query_dialogs_interaction_Z = self.cnn_query_dialogs(query_dialogs_interaction_simialrity_matrix)
        query_dialogs_interaction_V = self.affine_query_dialogs_interaction(query_dialogs_interaction_Z)
...
class ourModel (nn.Module):
    def __init__(self):
        ...
        self.embed_dim = 768
        self.query_profile_transformer  = TransformerBlock(input_size=self.embed_dim)
        self.query_dialogs_transformer  = TransformerBlock(input_size=self.embed_dim)
    ...
    def forward(self, batch_query_emb, batch_profile_emb, batch_dialogs_emb, \
        ...
        query_add_profile_attn = self.query_profile_transformer(batch_query_emb_norm, batch_profile_emb_norm, batch_profile_emb_norm, mask=~query_profile_attn_mask_)
        ...
        query_add_dialogs_attn = self.query_dialogs_transformer(batch_query_emb_norm, batch_dialogs_emb_norm, batch_dialogs_emb_norm, mask=~query_dialogs_attn_mask_)
