...
class DoctorRecDataset(Dataset):
    def __init__(self, split, dataset, profile, query, dialogue, first_sent, 
        ...
        self.first_sent_emb = first_sent
        train_set = pd.read_csv('./dataset/combined_train_cleaned.csv', delimiter='\t', encoding='utf-8', dtype={'dr_id':str})
    ...
    def __getitem__(self, index):
        ...
        # first_sent:
        first_sent = self.features[index][1 + self.dr_dialog_sample : -1]
        first_sent_mask = self.masks[index][1 + self.dr_dialog_sample : -1]

        # print("features: ", len(self.features[index]))
        # print("masks: ", len(self.masks[index]))

        # print("dialog: ", torch.FloatTensor(np.array(dialog)).shape)
        # # print("dialog_mask: ", dialog_mask)
        # print("dialog_mask: ", torch.FloatTensor(np.array(dialog_mask)).shape)
        
        # print("first_sent: ", torch.FloatTensor(np.array(first_sent)).shape)
        # print("first_sent_mask: ", torch.FloatTensor(np.array(first_sent_mask)).shape)
        ...
        return (torch.FloatTensor(profile), torch.FloatTensor(profile_mask), \
                ...
                torch.FloatTensor(np.array(first_sent)), torch.FloatTensor(np.array(first_sent_mask))), \
    ...
    def pack_dr_features(self, dr_id):
        ...
        if len(records) >= self.dr_dialog_sample:
            ...
            # add first sent into features
            for idx in sample_records:
                feature.append(self.first_sent_emb[idx]["embedding"])
                masks.append(self.first_sent_emb[idx]["attention_mask"])
        else:
            ...
            for idx in records:
                feature.append(self.first_sent_emb[idx]["embedding"])
                masks.append(self.first_sent_emb[idx]["attention_mask"])
            # padding for first_sent
            feature.extend([[0] * self.embed_size] * pad_size)
            masks.extend([[0] * self.first_sent_max_len] * pad_size)
